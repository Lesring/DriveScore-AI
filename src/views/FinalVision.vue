...
<template>
  <div class="min-h-screen relative overflow-hidden">
    ...
    <div class="absolute bottom-1/4 right-1/4 w-80 h-80 bg-secondary/30 rounded-full blur-3xl"></div>
    ...
    <main class="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 py-8">
      ...
      >
        <div class="inline-flex items-center gap-2 px-4 py-2 glass-card mb-8 rounded-full">
          ...
          <span class="text-sm text-white/80">The Future of Driving Sound</span>
        </div>
        ...
        <div class="glass-card p-12 max-w-4xl w-full mb-12">
          <div class="flex flex-col md:flex-row items-center justify-center gap-8">
            ...
            >
              ...
              <p class="text-white/50 text-sm text-center max-w-[180px]">{{ phase.description }}</p>
              <div class="text-xs text-white/30 mt-2">{{ phase.phase }}</div>
              
              <ArrowDown 
                v-if="index < finalVisionData.length - 1"
                class="w-6 h-6 text-white/20 mt-4"
              />
            </div>
          </div>
        </div>
        
        <div class="text-2xl md:text-3xl text-white/80 font-light italic mb-12">
